...
export default async function getData(request, response) {
  ...
  const nextUrl =
    skip + limit > length ? null : process.env.HOST_ADDRESS + "/api/v1/" + request.params.resource + `?limit=${limit}&skip=${skip + limit}`
  const previousUrl =
    skip === 0
      ? null
      : process.env.HOST_ADDRESS + "/api/v1/" + request.params.resource + `?limit=${limit}&skip=${skip - limit < 0 ? 0 : skip - limit}`
